...
class PlaylistGeneralSheetController: TransparentBottomSheetController {
	...
	private let actions: [SheetAction] = [.rename, .delete]
...
extension PlaylistGeneralSheetController: UITableViewDataSource, UITableViewDelegate {
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		self.removeSheet {
			switch action {
				case .rename:
					self.delegate?.didTapRename(self, playlist: self.playlist)
				case .delete:
					self.delegate?.didTapDelete(self, playlist: self.playlist)
				default:
					break
			}
		}
